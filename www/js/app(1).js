...
// ГЛАВНАЯ
// ============================================================
// Полоска счетов: «Всего» + каждый счёт/карта. Тап = фильтр всего экрана.
function accountStripHTML() {
...
async function openNotifications() {
  ...
  $('#btn-parse', m).onclick = () => {
    ...
    if (!parsed.length) { box.innerHTML = '<div class="tip warn"><div class="t-head">⚠️ Не распознано</div><div class="t-body">Не нашёл сумму с валютой (₽/руб/р). Проверьте текст.</div></div>'; return; }
...
function suggHTML(p) {
  ...
  const accLine = p.accountName ? ` · ${esc(p.accountName)}` : '';
  const balLine = p.balance != null ? ` · остаток ${S.fmtMoney(p.balance)}` : '';
  return `<div class="sugg-card" data-hash="${p.hash}">
    <div class="flex">
      <div class="tx-ico" style="background:${c.color}22">${c.icon}</div>
      <div class="grow">
        <div style="font-weight:700">${esc(p.note || c.name)}</div>
        <div class="muted" style="font-size:12px">${esc(c.name)} · ${p.type === 'income' ? 'доход' : 'расход'}${accLine}${balLine}</div>
      </div>
    ...
    card.querySelectorAll('button').forEach(btn => {
      btn.onclick = () => {
        ...
        if (act === 'add') {
          ...
          if (p.accountId) S.ensureAccount({ id: p.accountId, name: p.accountName, bankId: p.bankId });
...
function openTxModal(existing = null, draft = null, onSaved = null) {
  ...
  let accountId = src.accountId || (currentAccount !== 'all' ? currentAccount : 'main');
  if (!S.getAccounts().some(a => a.id === accountId)) accountId = 'main';
  const m = openModal(`
  ...
  $('#acc-row', m).querySelectorAll('.chip').forEach(el => el.onclick = () => {
    accountId = el.dataset.a;
    $('#acc-row', m).querySelectorAll('.chip').forEach(x => x.classList.toggle('active', x === el));
  });
...
function openAccounts() {
  const m = openModal(`
    <h2>💳 Счета и карты</h2>
    <div id="acc-list"></div>
    <button class="btn btn-block mt8" id="acc-add">＋ Добавить счёт</button>
    <p class="muted" style="font-size:12.5px;margin-top:10px">Счета банков создаются автоматически из уведомлений (Сбер •0150, ВТБ •5678…). Отметка ● — остаток по данным самого банка из SMS. Тап по счёту на главной фильтрует всю статистику.</p>
    <button class="btn btn-ghost btn-block mt8" id="acc-close">Закрыть</button>`);

  const renderList = () => {
    $('#acc-list', m).innerHTML = S.getAccounts().map(a => {
      const calc = S.balance(a.id);
      const real = a.balance != null;
      return `<div class="legend-row">
        <span class="legend-dot" style="background:var(--accent2)"></span>
        <span class="legend-name">${esc(a.name)}<br><span class="muted" style="font-size:11px">${real ? '● банк: ' + S.fmtMoney(a.balance) + ' · ' : ''}по операциям: ${S.fmtMoney(calc)}</span></span>
        <button class="btn btn-sm btn-ghost" data-ren="${a.id}">✏️</button>
        ${a.id !== 'main' ? `<button class="btn btn-sm btn-ghost" data-del="${a.id}">🗑</button>` : ''}
      </div>`;
    }).join('');
    m.querySelectorAll('[data-ren]').forEach(b => b.onclick = () => {
      const a = S.getAccount(b.dataset.ren);
      const m2 = openModal(`
        <h2>✏️ Счёт</h2>
        <div class="field"><label>Название</label><input id="ra-name" value="${esc(a.name)}"></div>
        <div class="flex"><button class="btn btn-block" id="ra-cancel">Отмена</button><button class="btn btn-primary btn-block" id="ra-save">Сохранить</button></div>`);
      $('#ra-cancel', m2).onclick = () => { closeModal(); openAccounts(); };
      $('#ra-save', m2).onclick = () => {
        const n = $('#ra-name', m2).value.trim();
        if (n) S.renameAccount(a.id, n);
        closeModal(); openAccounts();
      };
    });
    m.querySelectorAll('[data-del]').forEach(b => b.onclick = () => {
      if (confirm('Удалить счёт? Его операции перейдут в «Основной счёт».')) {
        if (currentAccount === b.dataset.del) currentAccount = 'all';
        S.deleteAccount(b.dataset.del);
        renderList();
      }
    });
  };
  ...
  $('#acc-add', m).onclick = () => {
    const m2 = openModal(`
      <h2>＋ Новый счёт</h2>
      <div class="field"><label>Название</label><input id="na-name" placeholder="Напр.: Наличные"></div>
      <div class="flex"><button class="btn btn-block" id="na-cancel">Отмена</button><button class="btn btn-primary btn-block" id="na-save">Добавить</button></div>`);
    $('#na-cancel', m2).onclick = () => { closeModal(); openAccounts(); };
    $('#na-save', m2).onclick = () => {
      const n = $('#na-name', m2).value.trim();
      if (!n) { toast('Введите название'); return; }
      S.addAccount(n); closeModal(); openAccounts();
    };
  };
  $('#acc-close', m).onclick = () => { closeModal(); renderCurrent(false); };
}
